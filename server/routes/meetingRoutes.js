...
router.post("/create", async (req, res) => {
  try {
    ...
    let meetingID = null;
    let is_active = null;
    if (meetingResp.recordset[0]) {
      meetingID = meetingResp.recordset[0].meeting_id;
      is_active = meetingResp.recordset[0].is_active;
    }
    // if active, send err
    if (is_active) {
...
router.get("/:meetingID/attendees", async (req, res) => {
  try {
    ...
    if (attendees.length == 0) {
      res
        .status(404)
        .send("There are no attendees for this meeting and event ID");
      return;
    }
